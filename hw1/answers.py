...
False. Not every split of the data produces a useful train–test split. A random split can create a situation where the training set does not include certain classes or scenarios that appear in the test set, or the sets may be too small to train or evaluate the model reliably.  
A good split must be representative, of adequate size, and sometimes stratified according to important classes or features.

False. The test set should not be used during cross-validation. Using it during CV can cause overfitting to the test set, leading to an unreliable estimate of the model’s generalization performance. The test set should only be used at the very end to evaluate the final model.

True. In cross-validation, the training set is split into \(k\) folds. In each round, one fold serves as a validation fold that is not used for training. Its performance reflects the model’s ability on unseen data, and averaging the results across all folds provides a reliable estimate of the generalization error.

True. Injecting noise into the labels creates situations where some of the data is inaccurate. A robust model should be able to learn the general patterns without relying on every single example. If the model overfits to the clean data, its performance will drop when noise is added.  
...
When we allow \(\Delta < 0\), this requirement is reversed.
...
When the data-dependent part of the loss is mostly zero, the only remaining term that influences optimization is the regularization term:
...
For an input image \(x\), it computes scores

\[
s_k = w_k^\top x,
\]

and predicts the class with the highest score.
...
In the accuracy graph, the training accuracy remains consistently a few percent higher than the validation accuracy.  
The gap is stable and small, indicating mild overfitting.  
In strong overfitting, validation accuracy would drop while training accuracy continued to rise — which does not happen here.
"""
...
Yes — it is still a linear regression model.  
...
No — we cannot fit any nonlinear function.  
...
Adding nonlinear features changes the classifier’s decision boundary:
