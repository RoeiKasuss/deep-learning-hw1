...
class FirstLastSampler(Sampler):
    ...
    def __init__(self, data_source: Sized):
        ...
        self.indices = list(range(self.data_source.offset, self.data_source.offset+ len(self.data_source)))
...
def create_train_validation_loaders(
    ...
    #  you create.
    # ====== YOUR CODE: ======
    validation_size = int(math.floor(validation_ratio * len(dataset)))
    train_subset = SubsetDataset(dataset, len(dataset) - validation_size)
    val_subset = SubsetDataset(dataset, validation_size, len(dataset)-validation_size)

    dl_train = torch.utils.data.DataLoader(
        train_subset, batch_size=batch_size, num_workers=num_workers,
        sampler=FirstLastSampler(train_subset))

    dl_valid = torch.utils.data.DataLoader(
    val_subset, batch_size=batch_size, num_workers=num_workers,
    sampler=FirstLastSampler(val_subset))
...
def create_train_validation_loaders(
    ...
    # ====== YOUR CODE: ======
    size = len(dataset)
    val_size = int(validation_ratio * size)
    ...
    dl_valid = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size,num_workers=num_workers, sampler=val_sampler)
    # ========================
